...
База данных состоит из 8 таблиц. Ядро — пользователи (`users`) с ролями организатора и участника, квизы (`quizzes`) с вопросами и вариантами ответов. Для реалтайм-сессий используются таблицы `quiz_sessions` (состояние комнаты, код, публичность, лимит участников) и `quiz_session_participants` (привязка участников к сессии с подсчётом баллов). Ответы участников хранятся в `participant_answers` со связью many-to-many на выбранные варианты. Аутентификация реализована через `refresh_tokens` с хешированием и TTL.
